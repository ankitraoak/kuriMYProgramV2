...
from kurimypyrogram.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~kurimypyrogram.Client.start`, :meth:`~kurimypyrogram.idle` and :meth:`~kurimypyrogram.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that kurimypyrogram's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`kurimypyrogram.compose`.
